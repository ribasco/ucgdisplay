...
abstract public class GlcdCommProtocol {
    ...
    public GlcdCommProtocol(ByteEventHandler byteEventHandler) {
        this.byteEventHandler = byteEventHandler;
    }

    protected void emitByteEvent(int data) {
        if (byteEventHandler != null) {
            byteEventHandler.onByteEvent(data);
        }
    }

    abstract public void decode(GpioEvent event);
}

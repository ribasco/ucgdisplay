 ...
 * =========================END==================================
 */
#ifndef UCGD_MOD_GRAPHICS_UCGDGPIOPERIPHERAL_H
#define UCGD_MOD_GRAPHICS_UCGDGPIOPERIPHERAL_H
...
class UcgdGpioPeripheral : public UcgdPeripheral {
...
public:
    ...
    explicit UcgdGpioPeripheral(const std::shared_ptr<UcgdProvider>& provider) : UcgdPeripheral(provider), log(ServiceLocator::getInstance().getLogger()) {}

    ~UcgdGpioPeripheral() override {
        debug("UcgdGpioPeripheral: destructor");
    };

    virtual void init(const std::shared_ptr<ucgd_t>& context, int pin, GpioMode mode) {
        if (!isModeSupported(mode)) {
            std::stringstream ss;
            ss << "init() : GPIO Mode '" << std::to_string(mode) << "' is currently not supported by the gpio provider (" << getProvider()->getName() << ")";
...
#endif //UCGD_MOD_GRAPHICS_UCGDGPIOPERIPHERAL_H

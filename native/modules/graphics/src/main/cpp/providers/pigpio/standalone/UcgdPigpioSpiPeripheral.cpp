...
UcgdPigpioSpiPeripheral::UcgdPigpioSpiPeripheral(const std::shared_ptr<UcgdProvider>& provider) : UcgdSpiPeripheral(provider) {}

UcgdPigpioSpiPeripheral::~UcgdPigpioSpiPeripheral() {
    debug("UcgPigpioSpiProvider : destructor called");
    for (auto const &device : m_OpenDevices) {
        if (device != nullptr && device->tp_spi_handle > -1) {
            spiClose(device->tp_spi_handle);
            device->tp_spi_handle = -1;
        }
    }
};

void UcgdPigpioSpiPeripheral::open(const std::shared_ptr<ucgd_t> &context) {
...
int UcgdPigpioSpiPeripheral::write(const std::shared_ptr<ucgd_t> &context, uint8_t *buffer, int count) {
    if (context->tp_spi_handle < 0) {

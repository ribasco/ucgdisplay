...
UcgdLibgpiodGpioPeripheral::UcgdLibgpiodGpioPeripheral(const std::shared_ptr<UcgdProvider> &provider)
        : UcgdGpioPeripheral(provider) {
}

UcgdLibgpiodGpioPeripheral::~UcgdLibgpiodGpioPeripheral() {
    debug("UcgLibgpiodGpioProvider : destructor called (no call to close)");
};

void UcgdLibgpiodGpioPeripheral::init(const std::shared_ptr<ucgd_t> &context, int pin, GpioMode direction) {
    //Do not process unassinged
    if (pin <= -1)
        return;

    std::shared_ptr<UcgdLibgpiodProvider> derived = std::dynamic_pointer_cast<UcgdLibgpiodProvider>(getProvider());
    const std::shared_ptr<gpiod::chip> &chip = derived->getChip();
...
void UcgdLibgpiodGpioPeripheral::write(int pin, uint8_t value) {
    //Ignore pins < 0
    if (pin <= -1)
        return;
    //GPIO Userspace code
...
gpiod::line *UcgdLibgpiodGpioPeripheral::findGpioLine(int pin) {
...
int UcgdLibgpiodGpioPeripheral::dirToInt(GpioMode direction) {
    switch (direction) {
...
bool UcgdLibgpiodGpioPeripheral::isModeSupported(const UcgdGpioPeripheral::GpioMode &mode) {
    switch (mode) {
